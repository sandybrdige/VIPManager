...
class Manager(QMainWindow, Ui_MainWindow):
    ...
    def QueryVipDetailcallback(self, model):
        self.tableView.setModel(model)#self.tableView.setItemDelegateForColumn(0, EmptyDelegate(self))
...
class MyDialog(QDialog, Ui_Dialog):
    ...
    def verifypw(self):
        ...
        if self.lineEdit.text() == self.pw:
            ...
            print("密码验证错误")
            self.label_3.setText("PW Error,Retry it")
...
class DataBase(QtCore.QThread):
    ...
    def handlevip(self):
        ...
        try:
            ...
            if value == list():
                c.execute(
                    "INSERT INTO VIPinfo VALUES(NULL,'%s','%s','%s','%s','%s','%s','%s')" %
                    (NAME, ID, PHONE, CONSUME, "SILVER", CONSUME, "1"))
                c.execute(
                    ('UPDATE VIPinfo SET VIPlV=CASE '
                     ...
                     'ELSE DISCOUNT END WHERE VIPID=%s') %
                    (ID))
                judge = c.execute(
                    "SELECT * from VIPinfo where VIPID=%s" %
                    (ID))
                ...
                c.execute(
                    "INSERT INTO CostDetail VALUES(NULL,'%s','%s','%s','%s','%s')" %
                    (NAME, ID, PHONE, VIPprize, TIME))
                self.sig.emit(mes)
    ...
    def selectVIPthid(self):
        ...
        try:
            judge = c.execute(
                "SELECT NAME,PHONE from VIPinfo where VIPID=%s" %
                (ID))
            value = judge.fetchall()
            NAME = value[0][1]
            PHONE = value[0][0]
    ...
    def selectVIPthphone(self):
        ...
        try:
            judge = c.execute(
                "SELECT VIPID,NAME from VIPinfo where PHONE=%s" %
                (PHONE))
            value = judge.fetchall()
            VIPID = value[0][1]
            NAME = value[0][0]
    ...
    def CreatCSVdetail(self):
        ...
        with open(filename, 'w', encoding='utf-8', newline='') as f:
            ...
            csv_wrtie.writerow(
                ('KEY', 'NAME', 'ID', 'PHONE', 'COST', 'COSTTIME'))
